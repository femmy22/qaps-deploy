// Component associated with the two unsymmetrical lines test.
...
function UnsymmetricalLines() {
	...
	async function onsubmitUnsymmetricalLinesHandler(drawingData) {
		// Adds the test type to the drawing data
		const unsymmetricalLinesData = {
			shape: 'Unsymmetrical Lines',
		...
		let navigationLink = await newData(unsymmetricalLinesData);
	...
	useEffect(() => {
		...
		function resizeShape() {
			...
			ctx.beginPath();
			ctx.lineWidth = 4;
			ctx.moveTo(newWidth * (2 / 5), newHeight * (1 / 5));
			ctx.lineTo(newWidth * (2 / 5), newHeight * (2 / 5));
			ctx.stroke();

			ctx.beginPath();
			ctx.lineWidth = 4;
			ctx.moveTo(newWidth * (3 / 5), newHeight * (4 / 5));
			ctx.lineTo(newWidth * (3 / 5), newHeight / 5);
			ctx.stroke();
		}
		...
		return (
			<div className={classes.testingTemplate}>
				{testSession.testSessionId ? (
					<Canvas onSubmit={onsubmitUnsymmetricalLinesHandler} />
				) : null}
...
export default UnsymmetricalLines;
